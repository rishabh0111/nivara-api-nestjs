...
The refresh cookie is `SameSite=None; Secure` wherever it is `Secure` at all, because a front end deployed on its own origin is cross-site with this API and `Lax` is not sent on a cross-site XHR. Sign-in would still work; refresh would not, so the session would simply end after fifteen minutes. What `Lax` used to refuse is refused instead by CORS, which grants credentials only to the configured front-end origins ([ADR-0003](docs/adr/0003-cors-split-by-credential-model.md)). Plain-http development keeps `Lax`, since `None` is discarded without `Secure` and `localhost` is same-site regardless of port.
